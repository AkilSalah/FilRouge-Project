    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
     ...
     <div class="grid grid-cols-1 md:grid-cols-1 gap-8 mt-10">
        ...
        <div class="flex max-lg:flex-col bg-white cursor-pointer rounded overflow-hidden shadow-[0_2px_10px_-3px_rgba(6,81,237,0.3)] hover:scale-105 transition-all duration-300">
          <img src="{{ asset($theme->image) }}" alt="Blog Post 1" class="lg:w-2/5 min-h-[250px] h-full object-cover" />
          <div class="p-6 lg:w-3/5">
            ...
            <span class="bg-blue-100 mt-16 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300"> {{$tag->tag}} </span>
            @endforeach
            <p class="text-sm mt-10">{{$theme->description}}</p>
            <div class="mt-10 flex justify-end items-end " >
                <button class="text-blue-500 hover:text-blue-700 mr-4"  data-modal-target="crud-modal-up-{{$theme->id}}" data-modal-toggle="crud-modal-up-{{$theme->id}}">Edit</button>
                {{-- ------------------------------------------------}}
                <div id="crud-modal-up-{{$theme->id}}" tabindex="-1" aria-hidden="true" class="mt-10 hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        <!-- Modal content -->
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <!-- Modal header -->
                            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t dark:border-gray-600">
                                ...
                                <button type="button" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-toggle="crud-modal-up-{{$theme->id}}">
                                    <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                                    </svg>
                                    <span class="sr-only">Close modal</span>
